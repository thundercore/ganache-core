      ...
      .on("end", function() {
        ...
        switch (method) {
          ...
          case "OPTIONS":
            ...
            break;
          case "GET":
            if (provider.options.ui) {
              switch (request.url) {
                case "/": {
                  headers["Content-Type"] = "text/html; charset=UTF-8";
                  const filePath = path.join(__dirname, "ui", "index.html");
                  const fileContents = fs.readFileSync(filePath, { encoding: "utf-8" });
                  sendResponse(response, 200, headers, fileContents);
                  break;
                }
                case "/mine": {
                  headers["Content-Type"] = "text/html; charset=UTF-8";
                  const filePath = path.join(__dirname, "ui", "mine.html");
                  const fileContents = fs.readFileSync(filePath, { encoding: "utf-8" });
                  sendResponse(response, 200, headers, fileContents);
                  break;
                }
                case "/style.css": {
                  ...
                  const filePath = path.join(__dirname, "ui", "style.css");
                  const fileContents = fs.readFileSync(filePath, { encoding: "utf-8" });
                  sendResponse(response, 200, headers, fileContents);
                  break;
                }
                case "/mine.js": {
                  ...
                  const filePath = path.join(__dirname, "ui", "mine.js");
                  const fileContents = fs.readFileSync(filePath, { encoding: "utf-8" });
                  sendResponse(response, 200, headers, fileContents);
                  break;
                }
